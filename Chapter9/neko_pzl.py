...
def game_main():
    ...
    if index == 0: #タイトルロゴ
        ...
        sc = sweep_neko() #消した数(num)が入る
        score += sc*difficulty*2 #スコア加算
        if score > hisc:
            hisc = score
        if sc > 0:
            index = 2
        ...
        if 24 <= mouse_x and mouse_x < 24 + 72 * 8 and 24 <= mouse_y and mouse_y < 24 + 72 * 10:
            ...
            if mouse_c == 1: #and neko[cursor_y][cursor_x] == 0:
                mouse_c = 0
                set_neko()
        ...
        if timer == 50:
            ...
            index = 0
    cvs.delete("INFO")
    draw_txt("SCORE "+str(score),160,60,32,"blue","INFO") #スコアを表示
    draw_txt("HISC " + str(hisc), 450, 60, 32, "yellow", "INFO")
    if tsugi >0 : #次の猫を表示
        cvs.create_image(753,128,image=img_neko[tsugi],tag="INFO")
